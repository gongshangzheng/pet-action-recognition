...
~~VideoMAEv2 K710 零样本~~ 降为参照（人类动作先验错位）。
~~A. VideoMAE v1 宠物微调版~~ **移出候选（2026-09-16 用户裁定）**：①有监督 7 类微调特征带判别偏置——对"发现标注表之外的新行为"有害；②微调产物可用性未经实际验证（此前仅凭 config 存在推断，核实不足）。注：其 **K400 预训练（未微调）权重**仍可作路线 W 骨干初始化备选，与微调版是两回事。
