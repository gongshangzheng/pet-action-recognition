...
```
E_mot（动作分支）：视频特征窗 → 动作编码器 → VQ(K=512 行为素) → 重建+速度匹配
                    精炼目标 = CatHuBERT 迭代伪标签 CE（L6，判别式）
E_id （身份分支）：视频特征窗 → 池化+MLP → s(64) → InfoNCE(track 级 ID 监督)
                    身份收进独立子空间（分流原则，供登记-检索支线复用）
对抗辅助：GRL 把身份从 E_mot 推开（L6b 第 2 档）——与 E_id 正向监督互补
```
