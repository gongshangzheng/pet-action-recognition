...
- [ ] 2.6 architecture.md（D4.6）：§0 完整结构总览表、§1–5 五阶段逐个详解（含否决方案与理由）、§6 跨切面（双环境/petlib/决策索引表）、§7 进度与未来计划四分类 + 闸门里程碑 + 二期对照
- [ ] 2.7 identity-tokenizer.md（D4.7 专篇）：§1 问题与定位、§2 FLOAT×TiTok 架构三要点、§3 两阶段路线、§4 评测矩阵、§5 训练配方、§6 验收裁定与回流、§7 环境依赖
...
- [ ] 3.4 架构与专篇与 change 原文一致性抽查：五阶段算法描述、否决理由、决策 ID 引用与各 design 原文一致；进度四分类与总管 tasks §2/§4 登记一致；tokenizer 专篇七节与 identity-action-tokenizer change 一致
