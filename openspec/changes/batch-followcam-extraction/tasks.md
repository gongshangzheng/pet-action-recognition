...
- [x] 1.6 批处理报告生成 + 告警段排查结案（pet:~/results/batch/batch_report.md + contact 接触表，本地 results/batch/）：低阈值复检 + 亮度/饱和度审计结论——三个告警段均为 **15s 超短片段 + 低猫在场率 + 红外域置信度边缘**（100505 漏检帧低阈值复检 0.10-0.18，部分帧猫确实不在场），**非算法故障**；处置 = 三段保留、行为时间轴如实标注在场率；语料事实修正：34 段实为短事件片段（总素材仅 **14.7 分钟**），行为发现主粮依赖 mammal_v0 + cats v1（~3 小时），34 段跟随视频承担管线验证与演示角色 → **任务 1.1-1.6 全部闭环，待用户归档** → 用户过目后 archive
