...
| 2.4 | `video-feature-latent` | 消费 2.3 编码器做行为发现 + 探针评测（现成 backbone 降为回退）；**并承接 L6 动作识别**（对 2.4b 切出的段贴标签：行为簇 + 命名） | 2.3 | ⏳ 待 2.3 |
| **2.4b** | **`video-action-segmentation`** | **行为素 → 动作段边界**（**只定边界、不贴标签**）：λ 能量预分段 + 细分段（变化点检测 / 无监督 TAS / 层次 HMM）| 2.4（且消费 2.3 的 λ）| 📋 已创建待启动（2026-09-17 立项） |
