...
### 10B. 参考图注册定物（调研完成 design D9，实施待用户决定）

- [ ] 10.5 （待批准）短期路线：per-camera ROI 登记（用户在画面上框一次）
- [ ] 10.6 （待批准）中期路线：DINOv2 登记照检索（与猫档案 register_cats.py 统一抽象）；OWL 图像引导检测作实验对照
