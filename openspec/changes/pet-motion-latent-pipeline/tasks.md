...
- [ ] 0A.1 单段白天视频 spike（`event_20260806_120311.mp4`，384 帧 15fps）：
  ...
  - 裁剪：每帧以平滑框中心裁 768×768（边长=1.2×max(w,h)，越界 clamp），写 `followcam.mp4`（15fps）
...
- [ ] 1.2 `scripts/make_followcam.py`：轨迹 JSON + 原视频 → `followcam.mp4`（768×768）+ 并排对比视频 + 接触表；参数：外扩 1.2×、平滑窗口 5、渐变速率上限（防跳切）
