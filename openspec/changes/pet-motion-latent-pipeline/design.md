...
- 居中裁剪：虚拟摄像机 = **位置连续跟随 + 尺寸锁定**（用户确认）。尺寸初值 = track 前 N 次检出的框 P75 × 1.2 外扩（数据驱动，示例段实测中位需 ~1234px@2880 宽），锁定后不变——**保留猫的表观尺寸变化**（远/近 = 距离线索，是行为信息的一部分）；保护规则：当实际框 > 窗口×0.9 时临时放大防裁切残肢，事件记入轨迹 JSON（放大次数也是"猫靠近摄像头"的行为信号）。窗口尺寸不再取固定 768 占位值（实测猫框中位 1028×633，768 会裁掉猫）
