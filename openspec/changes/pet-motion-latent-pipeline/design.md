...
### D1b: DetectionGate——可组合的检测门控组件（ sofas 误检漂移事故的设计回应；用户建议：门控思想作为通用组件供所有 SORT 系使用，而非独立候选跟踪器）
...
**位置与组合方式**：`petlib/detection/detection_gate.py`——**检测器与跟踪器之间的可组合过滤组件**（`DetectionGate.filter(detections, track_state) -> detections`），挂在任何跟踪器前面都能生效：ByteTrack/OC-SORT/BoT-SORT/DeepSORT 全部吃到门控收益。与阈值扫描的分工：阈值是全局一刀切（伤召回），门控是轨迹统计自适应（保召回杀误报）——两者正交，可叠加。消融矩阵：{四跟踪器} × {gate on/off}（任务 2.3/2.5 扩展）。
