...
① 背景移除（**`rembg` 类实时小模型**主选，见 pet-background-removal design D2；SAM 已移出抠像）
   ...
   ▼
② 待分析视频 → ViT 编码器（AdapTok 同形：12 层 / 768 维 / patch 4×8×8）
   │     ※ 无量化层——编码器输出即连续潜变量（T-A4b）
   ▼
TV tokens（每帧 N_TV 个）——「这一帧在动什么」
   │
   ▼
...
λ_m(t) 基元强度序列（可解释，副产品）
   ...
   │   └────────────────────────┘
   ▼
解码器 decode(w_identity, λ_t) → 重建第 t 帧
   损失：L1 + perceptual + adversarial（TivTok 口径）
   解耦验证：换参考输入 → 「B 做 A 的动作」（**免费**，见 T-A6）
```

> ⚠️ **上图为「参考输入」目标形态**（2026-09-17 用户裁定）。它与 T-A3 的原始 SIF 双 token 描述有**待解的连带问题**，见 T-A5c。

### T-A5c: 参考输入带来的连带问题（**待解**，2026-09-17 记录）

身份移到参考输入后，T-A3 的「视频内 TIV/TV 双 token」需重新推导：

| # | 问题 | 候选 |
|---|---|---|
| Q1 | 身份已由参考提供，**视频内的 TIV 还需要吗**？保留则与身份通道职责重叠 | ① 去掉视频内 TIV（只留动作通道，FLOAT 式）② 保留作存在性/自一致性校对 ③ 只用于参考输入编码，不在驱动视频上跑 |
| Q2 | **TV 当前 scope 是「只看本帧」，但运动必须有多帧上下文**——λ_t 怎么拿到时间上下文？ | ① TV 改为因果窗口 `[t-k, t]` ② λ 由独立时序模块预测（FLOAT 用 flow-matching transformer）③ TV attend 全段（但与 TIV 无区别） |
| Q3 | 参考编码器与视频编码器**是否共享权重**？ | 共享（参数省、域一致）/ 分开（职责分明）|
| Q4 | 参考为**多图/短视频**时如何聚合为一个 w_identity？ | 池化 / 注意力聚合 / 取均值 |

> **现状**：Q1 与 Q2 是**阻塞性**的——它们决定 T-A3 要不要改写。需用户裁定后再定稿。
