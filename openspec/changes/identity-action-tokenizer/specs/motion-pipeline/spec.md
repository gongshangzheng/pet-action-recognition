...
- **身份通道**：身份表示 MUST 从**参考输入**（单图 / 3–5 张多视角图 / 参考短视频）提取，MUST NOT 从待分析视频推断；读出 MUST 采用 **register tokens 跨全部参考 token 联合融合**（MUST NOT 用逐图独立编码后平均——那无法跨视角融合），融合结果投影为固定向量，使身份表示与参考形态、长度无关
