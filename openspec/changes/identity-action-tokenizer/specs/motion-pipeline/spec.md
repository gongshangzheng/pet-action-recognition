...
- **身份通道**：身份表示 MUST 从**参考输入**提取，MUST NOT 从待分析视频推断；**初版 MUST 只支持一种参考形态**（高质量多图 **或** 参考视频，二选一，见 《系统架构》 §2.5 **C13**）；读出 MUST 采用 **register tokens 跨全部参考 token 联合融合**（MUST NOT 用逐图独立编码后平均——那无法跨视角融合），融合结果投影为固定向量
