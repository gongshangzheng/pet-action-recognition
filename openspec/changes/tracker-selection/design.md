...
### D1d: 为什么重启本 change（2026-09-17，用户追问触发）

**原延后理由的缺口**：

| # | 问题 |
|---|---|
| 1 | **「单猫」是未核实的假设**——只出现在管线脚本注释里；批处理报告自己打了「**多猫？**」的问号且**从未回答** |
| 2 | 语料是 2 个家庭（`cui`/`jiang`），每户几只猫未知 |
| 3 | **否决与延后被混为一谈**——被否决的是「**全帧率检测 + ByteTrack**」（成本高 + v5 实测观感更抖）；而「**关联器只跑抽样帧**」这个低成本组合**从未被评估** |
| 4 | **单猫也有收益被漏掉**：离场/遮挡的 `lost` 语义（修掉插值「画穿墙直线」的已知短板）、误报抑制、多猫同框时锁定目标 |

**当前管线的静默失败**（若确有多猫）：

```python
# scripts/pet_detect_track.py —— 这就是全部的「关联」
if lab == CAT:
    if entry["cat"] is None or sc > entry["cat"][1]:
        entry["cat"] = (box, float(sc))     # 谁的置信度高取谁
```

两只猫都在时 → 框在两者间**跳变** → 插值 + 平滑把跳变铺成一条**平滑但荒谬的路径** → **不报错、只产出垃圾**，且下游行为发现会把它当数据。

**实验设计增补（相对 T1）**：

- **两组形态对比**：A. 关联器只跑抽样帧（新，检测次数不变、成本近乎为零）× B. 全帧率（对照）
- **是否用卡尔曼预测参与框平滑**：只贴 id（保观感）× 参与平滑（可能引入抖动）——后者需复核 D1 的「跟踪精度 ≠ 观感质量」
- **前置闸门**：多猫核实（tasks 0.x）——这是数据质量核实，结论可能要求重跑受影响的 followcam 段

---

### D1c: 候选跟踪器原理（§4A 参考；主链当前采用抽样+插值，见 D1）
