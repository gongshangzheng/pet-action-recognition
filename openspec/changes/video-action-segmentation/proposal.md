...
- **产出**：动作段列表（起止时间 + 类型），供 `spot-check-cli` 直接渲染为报告
...
- `motion-pipeline`: 新增「行为素序列 → 动作段」的分割要求——给定行为素/`λ` 序列，系统 SHALL 输出动作段列表（起止时间 + 类型），并 MUST 对「静止段」按**时长阈值**区分「停顿（分隔符）」与「静止型行为」

## Impact

- **管线新增一环**：架构 §4 分层设计新增 L5「动作分割」（原 L5 身份识别、L6 出口顺延）
