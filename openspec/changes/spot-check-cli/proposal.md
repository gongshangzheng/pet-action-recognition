...
**实时形态（2026-09-17 推理形态（原C18，已裁定） 已裁定，见架构 §1.3）**：实时性只落在**动作分段**上（流式、因果、秒级边界检出）；动作判别为**非流式**——分段模型检出边界后，把缓存帧**整段**交给判别模型一次判别（RTF<1 即可，无更高实时要求）。抽查与实时共用同一判别模型与行为簇字典，**不是两套模型**。
