    ...
    <n-card title="周报" size="small">
      ...
      <n-spin :show="loading">
        <n-table v-if="reports.length" :bordered="false" :single-line="false" size="small" striped>
          <thead>
            <tr><th>年份</th><th>周次</th><th>作者</th><th>标题</th><th>操作</th></tr>
          </thead>
          <tbody>
            <tr v-for="r in filteredReports" :key="r.id">
              ...
              <td>{{ r.author }}</td>
              <td>{{ r.title }}</td>
              <td><n-button text type="primary" @click="router.push(`/management/weekly/${r.year}/${r.week}/${r.author}`)">查看</n-button></td>
            </tr>
          </tbody>
        </n-table>
...
import { getWeeklyList } from '../../api/management'
...
onMounted(async () => {
  loading.value = true
  try { reports.value = await getWeeklyList() } catch { reports.value = [] }
  loading.value = false
})
</script>
