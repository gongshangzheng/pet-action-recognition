...
- 目录：`models/mmaction2/`
- 许可：见 `models/mmaction2/LICENSE`（Apache-2.0）
...
```bash
# 重新 shallow clone 覆盖
rm -rf models/mmaction2
git clone --depth 1 --single-branch https://github.com/open-mmlab/mmaction2.git models/mmaction2
SHA=$(git -C models/mmaction2 rev-parse HEAD)
rm -rf models/mmaction2/.git
# 更新本文件的「版本」行 + LICENSE 行，然后：
git add models/mmaction2 models/README.md
