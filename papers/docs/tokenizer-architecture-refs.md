...
**⚠️ 本项目定位（2026-09-17 用户裁定）**：OmniMate 的纯 in-context 做法作为**对照组（A0）**、**不是主线**——因为它**不产生身份潜变量**（没有 `w_id`），「同身份潜变量接近」这个要求无处施加。主线候选见架构 §2.5 **C25**（A1 3D patchify + register / A2 VAE latent + register → 1D tokens）。
