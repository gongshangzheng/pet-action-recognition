...
```bash
git log --since="上周一" --until="本周一" --pretty=format:"%h %ad %an | %s" --date=short
```

跑完按主线分类（live/speedrun/t12/training/management/docs/...），从中**挑出有决策价值或硬产出**的 commit。**不要只凭对话记忆**——对话记忆只覆盖你参与的部分，会漏掉团队其他成员完成的工作（典型漏报：其他主线完成的产品能力、训练/评测产出、Web 性能、基础设施改进等）。

过滤标准：

- ✅ 进周报：有数字 / 决策价值 / 用户可见的产物 / 主线状态变化
- ❌ 不进周报：nightly auto sync / 周报本身的改稿 commit / 单点琐碎 fix / 纯内部维护（skill 治理、文档排版）/ 第三方参考调研
