...
**适用场景**：在线调试模型、演示能力、临时观察某路摄像头的实时行为。**不是生产形态**——生产是抽查（详 [6 号《系统架构》§2.6](../wiki/architecture)）。
...
**相关文档**：`.claude/skills/live/`（操作指南）/ [10 号《第三方项目借鉴》](../wiki/third-party-notes)（pet-videos 借鉴细节）/ [6 号《系统架构》§2.6](../wiki/architecture)（Live 在管线中的角色）