...
|---|---|---|---|---|---|---|
| **cats v1** 家猫监控 | 552MB（8 zip） | 8 类检测 + 活动标注 | 双人3 批（蒋/崔/cui），中英文命名 | 🗄️ `datasets/cats/` | 在用 | 训练 + 评测 + 行为发现 |
...
**双人 3 批标注**——三个标注者（蒋/崔/cui）独立标注同一批视频，命名风格：

```
datasets/cats/
├── dataset_蒋.zip     159M    视频数据（蒋命名）
├── dataset_崔.zip     117M    视频数据（崔命名）
├── dataset_cui.zip    159M    视频数据（cui 命名）
├── annotation_蒋.zip   84K    标注（蒋命名）
├── annotation_崔.zip   32K    标注（崔命名）
├── annotation_cui.zip  32K    标注（cui 命名）
├── annotation_jiang.zip 84K   标注（蒋英文命名）
└── annotation_cui.zip  32K   （同上 cui 英文）
```
