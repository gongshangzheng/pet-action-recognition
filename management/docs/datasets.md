...
|---|---|---|---|---|---|---|
| **cats v1** 家猫监控 | 552MB（4 zip） | 8 类检测 + 活动标注 | 双人标注（蒋 = jiang、崔 = cui） | 🗄️ `datasets/cats/` | 在用 | 训练 + 评测 + 行为发现 |
...
**双人标注**——两个标注者（蒋/崔）独立标注同一批视频，**每位标注者对应一个视频包 + 一个标注包**（文件用拼音名 `cui` / `jiang`）：

```
datasets/cats/
├── dataset_cui.zip      117M    视频数据（崔拼音）
├── dataset_jiang.zip    159M    视频数据（蒋拼音）
├── annotation_cui.zip    32K    标注（崔拼音）
└── annotation_jiang.zip  84K    标注（蒋拼音）
```
...
**应对**：项目主推"无监督发现行为"路线（`video-feature-latent`）：让数据自己聚成簇，再人工命名。这正是 identity-action-tokenizer change 想用 **FLOAT 式参考输入 + 正交运动基**架构解耦的原因。
...
猫语料太少（34 段 = 14.7 分钟 + 3k clips），训不动视频生成模型。**解法**：先在 UCF101（13320 段，规模大两个数量级）验证"FLOAT 式身份-运动分解架构"可行性，再迁移到猫语料。
