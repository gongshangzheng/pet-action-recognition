...
**为什么不用 mmcv 改动**：GroundingDINO/HQSAM/ViTPose 依赖重且与 mmcv 约束冲突风险高（[6 号 §7](../wiki/architecture) D5），所以 pet 上另起 conda env `plf`，管线脚本以 subprocess + env 切换调用，产物落盘交接。
...
**相关文档**：[3 号《模型》](../wiki/models)（每个模型实测）/ [2 号《数据集全景》](../wiki/datasets)（喂什么数据）/ [6 号《系统架构》§7](../wiki/architecture)（跨切面与环境隔离）/ `.claude/skills/remote-servers/`（远程服务器）/ `.claude/skills/training/`（训练操作指南）