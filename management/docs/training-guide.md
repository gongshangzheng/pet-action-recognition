...
**为什么不用 mmcv 改动**：GroundingDINO/HQSAM/ViTPose 依赖重且与 mmcv 约束冲突风险高（[《系统架构》 §5.1](../wiki/architecture) D5），所以 pet 上另起 conda env `plf`，管线脚本以 subprocess + env 切换调用，产物落盘交接。
...
26 个模型（详 [《模型》《模型》§5](../wiki/models)）注册在 `web/src/api/training.js` 后端：
...
**相关文档**：[《模型》](../wiki/models)（每个模型实测）/ [《数据集全景》](../wiki/datasets)（喂什么数据）/ [《系统架构》《系统架构》§5.1](../wiki/architecture)（环境隔离）/ `.claude/skills/remote-servers/`（远程服务器）/ `.claude/skills/training/`（训练操作指南）