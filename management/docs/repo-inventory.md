...
| 8 | [身份-动作 Tokenizer 专篇](../wiki/action-model-design) | FLOAT 式参考输入 + 正交运动基（连续潜空间）|
...
| 资产 | 路径/位置 | 规模 | 状态 | 用途 |
|---|---|---|---|---|
| cats v1 家猫监控（双人标注） | 🗄️ `datasets/cats/` | 552MB（4 zip） | **已就绪** | 训练/评测/猫语料迁移 |
| cats zip 清单 | 🗄️ `datasets/cats/` | dataset_cui(117M) / dataset_jiang(159M) + annotation_cui(32K) / annotation_jiang(84K) | — | 视频原始数据 + 标注 |
...
- DB：`data/papers.db` 239 篇 / `paper_categories` 519 条（详见 [《数据集全景》](../wiki/datasets) §8 标注类目规范）
- 三类研究 JSON：
...
> 8 活跃 + 16 归档。一句话定位、进度、状态；详情见 [《系统架构》《系统架构》§9](../wiki/architecture) 与 [《仓库资产盘点》 §7 外部资产索引](#7-外部资产索引)。
...
| 🗄️ `third-party/` | kabr-tools / keypoint-moseq / pet-videos / PigDetect / remix-petra | 5 个第三方库 | 详 [《第三方项目借鉴》](../wiki/third-party-notes) |
