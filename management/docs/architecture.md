...
> 只列索引：**编号 + 名字 + 状态**（按号升序）。选项 / 判据 / 落点在对应 change；已决项见 sidecar `changelog` 与正文就地说明；v7 前的登记表全文归档于 `architecture.json` 附录。
> **跳号 = 该项已决**（编号不复用）；跨文档引用统一写「名字（Cxx）」。

**🔴 待审定（不定就开不了工）**

- **C23 背景对象层的触发信号取哪个**——虚拟裁剪窗口位移 / 真实机位运动
- **C27 相机数量与拓扑**——单机位 / 多机位协作
- **C28 报告里「没记录到行为」的时段怎么归类**——猫在场静止 / 出画 / 摄像头死角 / 被遮挡
- **C30 多张参考图或参考视频如何聚合成一个身份向量**
- **C31 解码器是否加参考特征路径**（FLOAT/LIA 的 `feats`）
- **C32 个体特有的动作风格归谁**——身份码 / `λ` / 明确忽略

**🟡 待定（不阻塞开工）**

- **C01 夜间红外处理方案**——专用模型 / 红外+RGB 混输 / IR 上色
- **C02 多猫场景掩码**——并集 / 逐猫分离
- **C03 视频编码器代码骨架**（不是结构复用）
- **C04 编码器初始化**——AdapTok / VidTok·LARP 权重 / 从零
- **C05 patch 尺寸 `t×p×p`**
- **C06 输入规格**——分辨率 / 窗口长度
- **C08 正交基元数 M / 维度 d / 符号**
- **C12 训练粒度**（依赖 C04）
- **C14 参考编码器与视频编码器是否共享权重**
- **C15 register token 数量 K**
- **C16 身份 embedding 模型**（检索路线，与 C13 无关）
- **C17 向量库**——numpy 暴力 / FAISS
- **C19 动作长度**——定长 / 不定长
- **C20 一个视频多个动作**
- **C22 异常检测机制**
...
- **C33 动作识别路线**——`λ` 到手后怎么变成类别
- **C34 分词层做法**——行为素 → 动作段
- **C35 `video-feature-latent` 阶段划分**
- **C36 ④⑤⑥ 内部顺序**——分割与表征先后可互换
