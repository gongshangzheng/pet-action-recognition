...
| 动作特征 | `z_j` | 第 j 个 tubelet（覆盖 t 帧）的连续特征 |
| 动作系数 | `λ_j = (λ_1…λ_M)` | 动作特征在 **M 个正交运动方向**上的坐标；**每 t 帧一份**（t = tubelet 时间跨度，默认 4）|
...
> **「实时」的语义（四轴 / 因果性 / 为什么 RTF<1 不等于实时）→ 见 §2.7.2。**
...
> ⚠️ **两个必须先破除的混淆**：
> 1. **输入变长 ≠ 输出变长**——A1 是「变长输入 → **定长输出**」：均匀采样把任意长视频压成 T 帧，再出 1 个标签
...
在线（A5）若要做 → 滚动窗口近似（延迟 = 窗长），见 §2.7.2 三层表
```

**所以"多段 + 时间戳"不是"要不要"的问题，而是输出形态决定的。**

#### §2.7.2 「实时」的语义：四个正交轴 + 因果性三层
...
| C09 | ✅ **已定** | `λ` 的时间粒度 | — | **= tubelet 粒度（每 t 帧一份）**，默认 t=4 → 16 帧得 4 个 `λ`。理由见 §2.7.3 |
...
> **概念见 §2.7**：§2.7.1 能力分层 A0–A5（本节 C19/C20 的背景）· §2.7.2「实时」的四轴与因果性 · §2.7.3 为什么 `λ` 不逐帧

> **概念（能力分层 A0–A5）见 §2.7.1**。
