...
- **对比学习（contrastive learning）**：核心思想——把"同一类"样本在特征空间里拉近、把"不同类"样本推远。**本项目里它是身份识别的统一原理**（positive invariance + negative discrimination）：猫 Re-ID 的 embedding 检索与物体实例识别都是该原理的实现。
  > ⚠️ **术语修正（2026-09-17）**：对比学习是**原理**，不等于必须用对比损失。身份-动作 Tokenizer 的身份通道已改为**参考输入条件化 + 重建**（不用 InfoNCE——判别式目标只保证"分得开"不保证"装得下"，见 [8 号](./identity-tokenizer.md) §4.6）。embedding 检索路线的**模型与向量库是待实测槽位**（非已定的 DINOv2+FAISS）。
...
flowchart TD
    ...
    subgraph S5["⑤ 身份识别"]
        E1[DINOv2 + FAISS 检索]
        E2["参考输入 → w_identity<br/>固定向量（不用 InfoNCE）"]
    end
...
| 身份通道 | **参考输入**（用户事先拍摄：单图/3–5 张多视角图/短视频）→ `w_identity` 固定向量；**不用 InfoNCE**（事实依据：FLOAT 全文 0 次）|
...
| Token 式 | **参考输入条件化**：`w_identity`（固定向量）作为解码条件；**不用 InfoNCE** | 与动作表征同源、交换验证免费 | ⏳ 随 L4 产出 |
