...
- **对比学习（contrastive learning）**：核心思想——把"同一类"样本在特征空间里拉近、把"不同类"样本推远。**本项目里它是身份识别的统一原理**（positive invariance + negative discrimination）：猫 Re-ID 的 embedding 检索与物体实例识别都是该原理的实现。
  > ⚠️ **术语修正（2026-09-17）**：对比学习是**原理**，不等于必须用对比损失。身份-动作 Tokenizer 的身份通道已改为**参考输入条件化 + 重建**（身份「给定」而非推断，监督由重建承担——见 [8 号](./identity-tokenizer.md) §4.6）。embedding 检索路线的**模型与向量库是待实测槽位**（非已定的 DINOv2+FAISS）。
...
flowchart TD
    ...
    R1["参考输入<br/>单图 / 多视角图 / 短视频"] --> M10["w_identity 固定向量<br/>身份「给定」而非推断"]
    M10 --> M9
    M7 --> M9
```
...
**v1 定位**：抽查为主（主产品是段列表报告）；实时用滚动窗口近似拿到秒级延迟（**零模型代价**）；**真流式（低延迟内核）留到明确需要时**（如 <0.5 s 告警）。
...
| # | 选择项 | 选项 | 倾向与理由 |
|---|---|---|---|
| ~~C14~~ | ~~身份监督形式~~ | — | **已定（2026-09-17）：身份无需独立损失**——参考输入 + 容量闸门 + 重建已足够；理由见 `identity-action-tokenizer/design.md` T-A5 |
...
| C23 | **参考输入形态（初版只支持一种）** | **A. 高质量多图**（3–5 张，用户挑清晰多视角）/ **B. 参考视频**（一段）| **待决策**（2026-09-17）：<br>**A** 优势 = **原理最干净**（身份本质是时间不变量，静态图与之语义完全对齐 → **架构上不存在参考运动污染**）、质量用户可控、编码便宜；代价 = 依赖用户挑图、拍不好则信息不全<br>**B** 优势 = 用户成本最低（随手录）、视角/光照变化更自然丰富、**与视频编码器天然同构**；代价 = ⚠️ **参考运动污染**（register 可能吸收参考运动，而 `w_identity` 会加到每帧）、需挑窗口、编码贵、质量不可控<br>**共同点**：两种形态**共用同一套 register 读出结构**，差异只在输入 token 是「多张静态图」还是「多帧视频」 → **不阻塞架构**，可先实现结构、形态后定。<br>**与 FLOAT 保证的关系**：**A 与 FLOAT 等价**（静态图自身无运动，不存在参考运动泄漏）；**B 多一个泄漏面**（参考自身运动可能被 register 吸进 `w_identity`）→ 需额外处理 + 专属审计 |
| C24 | **参考输入架构落地** | 视频内 TIV 去留 / 读出机制 / 编码器共享 | **Q1/Q2 已裁定**（去 TIV；编码器整段提特征）；**读出机制已定**（register 联合融合 → 投影成单向量）；余见 C25–C26 |
| ~~C25~~ | ~~`w_identity` 形状 vs 加法分解~~ | — | **已消解**（2026-09-17）：`w_identity` 就是单个 512 维向量，加法分解直接可用 |
| C26 | 参考编码器 vs 视频编码器共享权重 | 共享 / 分开 | **倾向共享**（须两次独立前向）；共享时用**两组 register tokens**（身份组 / 运动组）|
| C27 | **register token 数量 K** | — | 待消融（K 大→信息全但冗余，K 小→瓶颈）|
| ~~C27~~ | ~~register token 数量 K~~ | — | **已取消**（register 方案移除）|
| C28 | **解码器是否加参考特征路径**（FLOAT 的 `feats`）| ⓐ 补 / ⓑ 不补 / ⓒ 折中 | **已定 ⓑ 不补**（2026-09-17）——目标不是重建质量；不补反使身份码更完整，代价是泄漏压力略高 → 双向审计把门 |
| C29 | **α 的时序上下文** | ⓐ 逐帧单帧算（FLOAT 忠实版）/ ⓑ 加时序上下文 | 3D tubelet（t=4）折中 |
...
| # | 决策 | 选项 | 说明 |
|---|---|---|---|
| C18 | **推理形态**（对应 §2.6 的①流式性 + ③因果性）| ① 纯流式逐帧（需因果内核）/ ② **定长段（滚动窗口）** / ③ 混合 | **已定 ②**（2026-09-17）：模型内核就是 16 帧窗口 → 滚动窗口推理**训练/推理一致、零重训**，延迟 = 窗长；抽查用全局非因果后处理（更准），实时用因果在线后处理（有延迟）。真低延迟内核（<窗长）需因果注意力 + 重训，**v2 条件启动** |
...
为避免"看起来已定、实际未实测"的图坑（历史教训：DINOv2+FAISS 曾被写得像已定，依据只是"预计够用"），全局统一三档标注：
