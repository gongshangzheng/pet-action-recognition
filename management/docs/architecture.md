...
| 身份通道 | **参考输入**（用户事先拍摄：单图/3–5 张多视角图/短视频）→ **register tokens 读出**（拼接可学习 register → 编码器 → 只保留 register tokens）|
...
| C23 | **参考输入形态** | 单张全身图 / 3–5 张多视角图 / 参考短视频 | **三种均允许且共用一套读出结构**（register tokens）——形态差异只在输入 token 数（2026-09-17）|
| C24 | **参考输入架构落地** | 视频内 TIV 去留 / 读出机制 / 编码器共享 / K 取值 | **Q1/Q2 已裁定**（去 TIV；编码器整段提特征）；**读出机制已定**（register tokens）；余见 C25–C27 |
| C25 | **`w_identity` 形状 vs 加法分解** | ⓐ register 投影成单个 d 维 / ⓑ 运动也 K 个 token / ⓒ 解码器 cross-attention 读出 | **倾向 ⓒ**（形状自由，两边各自发展）|
| C26 | 参考编码器 vs 视频编码器共享权重 | 共享 / 分开 | **倾向共享**（结构已统一）+ 两组 register tokens（身份组/运动组）|
| C27 | **register token 数量 K** | — | 新超参（**非**原 N_TIV，用途不同）；待消融 |
| C28 | **解码器是否加参考特征路径**（FLOAT 的 `feats`：源图多尺度特征逐层注入做 warp）| ⓐ 补（忠实 FLOAT，分离强）/ ⓑ 不补（靠容量闸门 + 审计）/ ⓒ 只中低分辨率 | **待裁定**（不加则外观只能从压缩身份码出，运动通道更有动机携带外观 → 分离压力弱于 FLOAT）|
...
| 2026-09-16 | 方案否证记录：参考帧机制、双独立编码器、模型内 VQ 码本（~~纯 register tokens~~ 已于 09-17 修正为采用）| L4 |
