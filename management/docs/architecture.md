...
> **怎么读**：**§1 端到端设计总图**（整体长什么样）→ **§2 当前待决策**（还剩什么没定）→ **§3–§10 按环节逐一讲解**（①定位跟踪 → ②跟随视角 → ③背景移除（optional）→ ④动作表征 → ⑤分割 → ⑥识别 → ⑦身份 → ⑧出口；每个环节自带它需要的概念）。
>
...
**一句话总览**：先**门控有无猫**（无猫早退）→ 得到以猫为中心的视频流→ 抠掉背景 → 学可分解的动作表征（`λ`）→ **先分段（找边界）再识别（贴标签）** → 结合身份输出「某时段某猫在做什么」的报告。
