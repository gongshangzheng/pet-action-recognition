...
> **编号连续（C01–C29），按号升序**；标 🔴 的 6 项待审定（不定就开不了工）：**C16 · C20 · C21 · C23 · C24 · C25**，其余为 🟡 待定（不阻塞开工）。选项 / 判据 / 落点在对应 change；已决项见 sidecar `changelog`；旧登记表全文与旧→新编号映射见 `architecture.json` 附录。

- **C01 夜间红外处理方案**——专用模型 / 红外+RGB 混输 / IR 上色
- **C02 多猫场景掩码**——并集 / 逐猫分离
- **C03 视频编码器代码骨架**（⚠️ 不是结构复用）
- **C04 编码器初始化**——AdapTok / VidTok·LARP 权重 / 从零
- **C05 patch 尺寸 `t×p×p`**
- **C06 输入规格**——分辨率 / 窗口长度
- **C07 正交基元数 M / 维度 d / 符号**
- **C08 训练粒度**（⚠️ 依赖 C04）
- **C09 参考编码器与视频编码器是否共享权重**
- **C10 register token 数量 K**
- **C11 身份 embedding 模型**（检索路线）
- **C12 向量库**——numpy 暴力 / FAISS
- **C13 动作长度**——定长 / 不定长
- **C14 一个视频多个动作**
- **C15 异常检测机制**
- 🔴 **C16 背景对象层什么时候重新分割**——画面里看到的区域变化超阈值（画面裁剪模拟的窗口移动 / 相机转动）+ 超时兜底
- **C17 掩码的类别来源**——SAM 只出掩码、不给类别
- **C18 过分割过滤**——自动分割会出大量碎片掩码
- **C19 背景对象层与 GDINO 家具框的关系**
- 🔴 **C20 相机数量与视野覆盖**——单机位 / 多机位；猫能否离开视野
- 🔴 **C21 报告里「没记录到行为」的时段怎么归类**——猫在场静止 / 出画 / 死角 / 被遮挡
- **C22 参考身份通路**——三方案消融，含对照组
- 🔴 **C23 多张参考图或参考视频如何聚合成一个身份向量**
- 🔴 **C24 解码器是否加参考特征路径**（FLOAT/LIA 的 `feats`）
- 🔴 **C25 个体特有的动作风格归谁**——身份码 / `λ` / 明确忽略
- **C26 动作识别路线**——`λ` 到手后怎么变成类别
- **C27 分词层做法**——行为素 → 动作段
- **C28 `video-feature-latent` 阶段划分**
- **C29 ④⑤⑥ 内部顺序**——分割与表征先后可互换
...
| 报告语义 | 不在场区间 MUST 在报告中显式标注（**不得计为"猫静止无动作"**）→ 见 **C21**（空白时段归类）与 **C20**（视野覆盖）|
...
| ④ 表征 · 窗口步长 | stride | **待定**（重叠 vs 不重叠）| 见 **C05（patch 尺寸 t）/ C13** |
...
**⭐ 对本项目的推论**：**多时间尺度变便宜了**——同一套权重换窗口大小只差一次微调 ⇒ **C13「多时间尺度」可用「同一骨干的不同窗口变体」实现，不必两套架构**。
...
2. **跨窗口独立**：相邻窗口 λ 独立算（除非重叠）→ 需后处理平滑（= C14 选项②）
3. 若要「**模型内部**处理任意长序列」→ 需因果/流式或长窗注意力 → 分段组件需流式（见 §10）；窗口跨度属 C13
...
| **本项目额外要做的** | ① 参考 = 视频（多参考聚合）② 3D t 帧块（运动需多帧）③ 参考身份通路（见 C22 / C23 / C24）|
...
| **动作与身份有交互**（个体特有动作）| **直接监督** | 解耦把该信息清掉了 ← 即 **C25** |
...
> ✅ **但「变长输出」可以用定长模型 + 滑窗做出来**：定长窗口铺满长段 → 每窗一个标签 → 后处理合并成段。**这正是 C14 选项② 的路线**——本质是「**用 A1 能力模拟 A3/A4 的输出形态**」，模型本身仍是 A1。
...
> ① **逻辑顺序**——不分段就不知道该对哪一段做识别；② **方法族完全不同**——分段是几何/时序问题（变化点检测 / HMM / 能量曲线，**纯无监督、不需要词表**），识别是语义问题（每段 → 类别，**需要词表**）；③ 拆开后 C27（分词层做法）与 C26（动作识别路线）才能分开讨论。
...
| **NLP unigram 分词范式** | 用动作词表对序列做**最优切分**（最大化 `Σ log P(段) + log P(长度)`），DP/Viterbi 求解 | C14 选项② 的严格版 |
...
| 类别来源（**三选，见 C26**）| **① 无监督**：对段嵌入聚类 → 行为簇 → 人工命名（主线）<br>**② 有监督**：片段级人工标注 → 训判别模型（C26 路线②）<br>**③ 聚类辅助 + 监督**：聚类降标注量，再用标注训分类器（C26 推荐）|
| 方法族 | 段嵌入 + 分类器：线性探针（验收闸门）/ 小 Transformer / n-gram / 语言模型式序列模型 |
| 与 L5 的区别 | L5 = **找边界**（不需要词表）；L6 = **贴标签**（需要词表）|
| 词表对齐 | 与 `video-feature-latent` 的行为簇命名表**共用或收敛为同一套**（见 C28）|
| 状态 | ⏳ 由 `video-action-segmentation`（2.4b）与 `video-feature-latent`（2.4）协作交付 |

> **为什么类别来源是"三选"**：分段不需要标签，但识别一定需要"类别的定义从哪来"。我们的 cats v1 activity 标注是**伞类**（一半以上），直接监督会学到伪类别 ⇒ 主线走①（聚类+命名），但**片段级人工标注是可行备选**（C26）。
