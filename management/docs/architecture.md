...
flowchart TD
    ...
    subgraph S1["① 定位与跟踪"]
        A1[GroundingDINO 抽样检测] --> A2[插值<br/>可选运动校正 v5]
    end
...
| 算法 | GroundingDINO 多 prompt 抽样检测（每 10 帧）→ 插值 → **（可选）运动校正 v5**（默认禁用） |
...
**运动校正 v5 是可选后处理**：猫被柱子/家具遮一下又出来、重新框到时位置可能跳变，运动校正让位置平滑。它是**插值之后的独立开关，默认禁用**（治标有限，关键帧丢就让它丢，渲染层过滤兜底）。参数与实测见 [9 号《研究结论与踩坑》§4.1](./lessons.md)。展开说明见 [7 号 §1](./identity-and-retrieval.md)。
...
| 2026-09-15 | 运动校正 v5 参数定稿（可选开关，默认禁用）| L1 |
