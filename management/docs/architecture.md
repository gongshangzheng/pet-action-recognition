...
| C23 | **参考输入形态（初版只支持一种）** | **A. 高质量多图**（3–5 张，用户挑清晰多视角）/ **B. 参考视频**（一段）| **待决策**（2026-09-17）：<br>**A** 优势 = **原理最干净**（身份本质是时间不变量，静态图与之语义完全对齐 → **架构上不存在参考运动污染**）、质量用户可控、编码便宜；代价 = 依赖用户挑图、拍不好则信息不全<br>**B** 优势 = 用户成本最低（随手录）、视角/光照变化更自然丰富、**与视频编码器天然同构**；代价 = ⚠️ **参考运动污染**（register 可能吸收参考运动，而 `w_identity` 会加到每帧）、需挑窗口、编码贵、质量不可控<br>**共同点**：两种形态**共用同一套 register 读出结构**，差异只在输入 token 是「多张静态图」还是「多帧视频」 → **不阻塞架构**，可先实现结构、形态后定 |
