...
flowchart TD
    ...
    subgraph S5["⑤ 动作分割（行为素 → 动作段）"]
        G1["λ 运动能量预分段<br/>→ 行为块"] --> G2["无监督 TAS<br/>细分段 + 类型聚类"]
        G2 --> G3["人工命名<br/>→ 段列表"]
    end
...
**一句话总览**：把监控视频切成「以猫为中心的小片」→ 抠掉背景 → 学一个可分解的动作表征（`λ`）→ **把 `λ` 序列分割成动作段** → 用段列表 + 身份做检索/异常检测，最终产品是「某时段某猫在做什么」的报告。
...
| 输出 | 动作段列表 `[{start_sec, end_sec, action_type, confidence, n_primitives}]` |
| 流程 | **三步**：① `λ` 运动能量预分段（零成本，`Σ_m\|λ_m(j)\|`）→ 切成「行为块」 ② 块内**无监督 TAS** 细分段 + 类型聚类 ③ 人工命名 |
| 静止段处理 | **按时长阈值分流**：`< T_pause` = 停顿（分隔符，不单独成段）；`≥ T_pause` = **静止型行为**（睡觉），参与聚类 |
