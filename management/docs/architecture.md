...
| C23 | **参考输入形态** | 单张全身图 / 3–5 张多视角图 / 参考短视频 | **三种均允许**：单图直接用；多图/视频逐图编码后**取平均**（2026-09-17）|
| C24 | **参考输入架构落地** | 视频内 TIV 去留 / 读出机制 / 编码器共享 | **Q1/Q2 已裁定**（去 TIV；编码器整段提特征）；**读出机制已定**（FLOAT 式全局向量）；余见 C25–C26 |
...
| 2026-09-16 | 方案否证记录：参考帧机制、双独立编码器、模型内 VQ 码本（register tokens 一条经 09-17 两次修正，最终简化为 FLOAT 式全局向量）| L4 |
