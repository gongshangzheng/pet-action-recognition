...
> **只收目标级与系统级决策**：模型内部（④ Tokenizer）的待决策在 [8 号《身份-动作 Tokenizer》](./identity-tokenizer.md) 末表管理（🔴 3 + 🟡 9）；具体参数（阈值 / 间隔 / 数值）属对应 change 的实验测定项，均不进本表。已决项见 sidecar `changelog`；历史编号映射见 `architecture.json` 附录。
...
- **背景对象层重分割的阈值与兜底间隔**——触发机制已定（看到的区域变化超阈值 + 超时兜底，2026-09-17）；待定的只是阈值数值与兜底间隔，属实验测定项（落点 `background-object-memory`）
...
```mermaid
flowchart LR
    V[跟随视角视频流] --> F["④ 表征（滑窗）<br/>每 t 帧一份 λ"]
    F --> S2["在 λ 序列上找边界<br/>（类比语音找分段符 / VAD）"]
...
| 报告语义 | **空白时段三分法（已裁定 2026-09-17）**：① 在场静止 = 动作类别「静止」（进行为词表，正常输出）② 出画 / 死角 = 不处理（门控早退，报告显式标注「不在场」，**不得计为静止**）③ 被遮挡 = 标注「无法测量」状态（测量状态，非动作类别；遮挡判定属实现细节）|
...
```
触发（机制已定，2026-09-17；阈值与超时间隔为实验测定项）：
...
| `λ_j = (λ_1…λ_M)` | 动作系数 | `z_j` 在这 M 个方向上的**坐标**；**每 t 帧一份**（当前 t=4 ≈ 0.27 s/份 @15fps；消融 t∈{2,4,8} 属 8 号「patch 尺寸」待定项）|
...
| ④ 表征 · 窗口步长 | stride | **待定**（重叠 vs 不重叠）| 见 8 号「patch 尺寸」与 §2「动作长度」 |
...
> ④ 的输入是**变长视频**；本节讲输入侧怎么处理。输出能力分类（A0–A5）见 §8⑤ 的能力分层。
...
3. 若要「**模型内部**处理任意长序列」→ 需因果/流式或长窗注意力 → 分段组件需流式（见 §11）；窗口跨度属「动作长度」
...
| **本项目额外要做的** | ① 参考 = 视频（多参考聚合）② 3D t 帧块（运动需多帧）③ 参考身份通路（见 8 号 🔴 参考聚合与 feats 路径两待审项，及 8 号「参考身份通路」消融）|
...
| **动作与身份有交互**（个体特有动作）| **直接监督** | 解耦把该信息清掉了 ← 即 8 号 🔴「个体特有的动作风格归谁」 |
...
| 跨个体泛化重要？ | ✅ 多猫 + 多环境 | 解耦有利 |
| 动作与身份有交互？ | 🔴 待审定（8 号：个体动作风格归属）| 潜在代价 |
