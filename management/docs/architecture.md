...
> **怎么读**：**§1 端到端设计总图**（整体长什么样）→ **§2 当前待决策**（还剩什么没定）→ **§3–§10 按环节逐一讲解**（①定位跟踪 → ②跟随视角 → ③背景移除（optional）→ ④⑤⑥ 动作理解（表征 · 分割 · 识别，顺序可互换）→ ⑦身份 → ⑧出口；每个环节自带它需要的概念）。
>
...
> 按**状态**分两组：🔴 **待审定**（不定就开不了工）与 🟡 **待定**（不阻塞开工）；组内按**层级**排列——**系统 / 管线级**（跨环节的部署与管线形态）· **模型级**（④ 身份-动作 Tokenizer 内部，深设计在 [8 号](./identity-tokenizer.md)）· **识别与身份级**（⑥ 类别落地与 ⑦ 检索）。选项 / 判据 / 落点在对应 change；已决项见 sidecar `changelog`；历史编号映射见 `architecture.json` 附录（2026-09-17 起不再使用编号）。

### 🔴 待审定（不定就开不了工）

**系统 / 管线级**

- **背景对象层什么时候重新分割**——触发 = 画面里看到的区域变化超阈值（画面裁剪模拟的窗口移动 / 相机转动）+ 超时兜底；要定的是阈值与兜底间隔
- **相机数量与视野覆盖**——单机位还是多机位；猫能否走出视野（出画 / 死角）；多机位才有视野重叠交接与盲区报告问题
- **报告里「没记录到行为」的时段怎么归类**——猫在场静止 / 出画 / 摄像头死角 / 被遮挡是四种不同含义，报告必须能区分

**模型级（④ Tokenizer）**

- **多张参考图或参考视频如何聚合成一个身份向量**——平均池化 / 注意力加权 / 逐图残差再聚合（LIA 机制的直接推论）
- **解码器是否加参考特征路径**（FLOAT/LIA 的 `feats`）——不加：身份码可能存不下外观细节；加：解码器变复杂、多一条泄漏面
- **个体特有的动作风格归谁**——「这只猫喝水总是慢」这类信息放身份码、`λ`、还是明确忽略（Slot-ID 引出）
...
- **多猫场景掩码**——两猫同框时抠像给并集还是逐猫分离（影响身份训练的「单猫片段」切分）
- **异常检测机制**——在 `λ` 空间做统计离群 / 与正常库比距离 / 重建误差
- **掩码的类别来源**——SAM 只出掩码不给类别，类别从哪来（GDINO 框 / 人工 / 规则）
- **过分割过滤**——SAM 自动分割会出大量碎片掩码，怎么过滤（面积 / 稳定性 / 出现时长阈值）
- **背景对象层与 GDINO 家具框的关系**——两套背景物体来源怎么合并去重
- **动作长度**——按定长窗口切还是不定长段；多时间尺度怎么实现（定长 + 同骨干不同窗口变体）
...
- **④⑤⑥ 内部顺序**——先分割再表征还是先表征再分割（可互换，见总图大框双向连线）

**模型级（④ Tokenizer）**

- **视频编码器代码骨架**——从 AdapTok / VidTok / LARP 借哪家的代码当骨架（⚠️ 只借实现、不借结构）
- **编码器初始化**——用 AdapTok / VidTok·LARP 的预训练权重起步，还是从零训练（决定分阶段冻结训练是否可行）
- **patch 尺寸 `t×p×p`**——3D patch 的时间跨度 t 与空间大小 p（当前 t=4、p=8；决定 `λ` 的时间粒度，消融 t∈{2,4,8}）
- **输入规格**——分辨率与窗口帧数（当前 512×512 / 16 帧稀疏采样）
- **正交基元数 M / 维度 d / 符号**——运动基用几个方向、每维多宽、系数有无非负约束（当前 M≈20–32）
- **训练粒度**——分阶段训练时冻结哪些模块（先冻结编码器训解码头，还是全联合；依赖「编码器初始化」）
- **参考编码器与视频编码器是否共享权重**——对称架构下两路是否同一套参数（当前主线：共享）
- **register token 数量 K**——身份 register 用几个 token 装
- **参考身份通路**——参考视频怎么喂给模型（三方案消融，含对照组）

**识别与身份级**

- **身份 embedding 模型**——检索路线用哪个模型把参考图变成向量
- **向量库**——身份向量存哪、怎么查（numpy 暴力检索 / FAISS）
- **动作识别路线**——`λ` 序列到手后怎么变成动作类别（无监督聚类+命名 / 有监督判别 / 混合）
...
| 报告语义 | 不在场区间 MUST 在报告中显式标注（**不得计为"猫静止无动作"**）→ 见 🔴「空白时段归类」与「视野覆盖」|
...
**待定项**：见 §2 速览 🔴「背景对象层什么时候重新分割」与 🟡 掩码三件（类别来源 / 过分割过滤 / 与 GDINO 家具框的关系）。
...
| `λ_j = (λ_1…λ_M)` | 动作系数 | `z_j` 在这 M 个方向上的**坐标**；**每 t 帧一份**（当前 t=4 ≈ 0.27 s/份 @15fps；消融 t∈{2,4,8} 属「patch 尺寸」待定项）|
...
| ④ 表征 · 窗口步长 | stride | **待定**（重叠 vs 不重叠）| 见「patch 尺寸」与「动作长度」 |
...
3. 若要「**模型内部**处理任意长序列」→ 需因果/流式或长窗注意力 → 分段组件需流式（见 §10）；窗口跨度属「动作长度」
...
| **本项目额外要做的** | ① 参考 = 视频（多参考聚合）② 3D t 帧块（运动需多帧）③ 参考身份通路（见 🔴 聚合与 feats 两待审项，及 🟡「参考身份通路」消融）|
...
| **动作与身份有交互**（个体特有动作）| **直接监督** | 解耦把该信息清掉了 ← 即 🔴「个体特有的动作风格归谁」 |
...
| 类别来源（**三选，见「动作识别路线」**）| **① 无监督**：对段嵌入聚类 → 行为簇 → 人工命名（主线）<br>**② 有监督**：片段级人工标注 → 训判别模型（路线②）<br>**③ 聚类辅助 + 监督**：聚类降标注量，再用标注训分类器（C26 推荐）|
...
> **已定（2026-09-17，见下文实时形态）**：按组件拆——分段流式、判别非流式（RTF<1 软要求）；原四选一作废；「低延迟内核」v2 条件项取消。
