...
| ID | 状态 | 选择项 | 选项 | 倾向 / 判据 |
|---|---|---|---|---|
| C13 | ✅ **已定：B（参考视频）** | ~~A. 高质量多图 / B. 参考视频~~ | — | **选 B**（用户裁定 2026-09-17）：① 对称架构下参考也走**同一套分解** → **「参考运动污染」不再是问题**（见 §1.5.1）；② 视频能覆盖猫**前后左右全部方向**（可要求用户拍得标准）→ 身份更完整；③ 与视频编码器**天然同构**（一套结构两个输入）。<br>~~原 A 方案~~（高质量多图）的"原理最干净"优势已被对称架构取代 |
