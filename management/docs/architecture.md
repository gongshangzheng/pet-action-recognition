...
| 量化器 | **无量化**（连续潜变量 + KL 正则）；理由与 VQ 利弊见 8 号《身份-动作 Tokenizer》§5.4b | 2026-09-17 | `identity-action-tokenizer` T-A4b |
...
| `λ` 的时间粒度（原 C09）| = **tubelet 粒度**（每 t 帧一份）| 2026-09-17 | 8 号《身份-动作 Tokenizer》§5.2a；**是 C05（patch `t`）的直接推论，非独立决策** |
...
| **2026-09-17** | **实时形态裁定（C18/C21）**：实时性只落在动作分段（流式因果）；判别非流式（RTF<1 软要求）；否决帧级流式识别；「低延迟内核」v2 取消 | §1.3 / §6.8 |
| **2026-09-17** | **章节重构（用户裁定）**：模型内部设计（聚合粒度 / 离散化 / 对称架构）移至 8 号；完整流程升为独立 §4；**总图前置到登记表之前**；登记表等顺延为 §5–§11；§1.1/§1.2 错位内容归位 | 全篇 |
