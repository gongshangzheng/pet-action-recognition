...
### §4.2a 聚合粒度：为什么每 t 帧一份动作系数（C09 已决）
...
**时间粒度**：`λ` 不是一帧一份，而是**每 t 帧聚合一份**（t = tubelet 时间跨度，默认 4 → 16 帧窗口得 4 份）。
...
**聚合粒度与 3D patchify 是同一个决定**：选 3D patchify，就是为了让模型以 t 帧（t = tubelet 时间跨度）为单位处理时间——tubelet 覆盖 t 帧 → 编码器每个时间位置输出一份，`T/t` 份动作系数（T = 窗口总帧数）。不是「架构碰巧对齐」，而是先有聚合需求，才选这个架构。

**为什么 tubelet 粒度够用**：动作基元曲线够密（0.27 s/点 @15fps）· 段边界 ±1.5 s 要求远宽于该粒度 · 行为聚类本就不需帧级 · 时间维 token 降至 1/t
...
| 4 | **block-causal attention** | ❌ 不再需要：C18 已定（2026-09-17）判别非流式、无需因果内核；分段模型的流式需求在 `video-action-segmentation` 另议 |
...
> **2026-09-17 修订（用户提议）**：参考与输入视频**共用同一套结构与参数**（FLOAT 即如此），差别只在**取哪部分输出**——参考侧取 register（身份），输入侧取 tubelet 系数（动作）。完整版见下文 §4.6b。
>
...
一句话：**参考视频和待分析视频走同一个模型，差别只在取哪个输出口**——参考取 register（身份），输入取 tubelet 系数（动作）。
...
| **C18** | ✅ | **推理形态** | **已定（2026-09-17）**：分段流式 + 判别非流式（RTF<1 软要求）；否决帧级流式识别。见 6 号 §1.3 |
| C19 | 🟡 | 动作长度（定长 / 不定长）| — |
| C20 | 🟡 | 一个视频多个动作 | — |
