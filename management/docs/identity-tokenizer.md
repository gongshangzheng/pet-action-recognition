...
| 4 | **block-causal attention** | 若将来走**流式/因果推理**（6 号 §4.6 **C18** 中「延迟 < 窗长」的变体）——AdapTok 现成 |
...
| C09 | 🟡 | `λ` 的时间粒度 | 3D patchify 张力，见 §4.2 |
| C10 | 🟡 | `α` 的时序上下文 | — |
| C11 | 🟡 | 对齐教师（DeRA）| 见 §7 可选稳定器 |
| C12 | 🟡 | 训练粒度（A1 冻结 / A1+A2）| 见 §5 训练流程 |
| **C13** | 🔴 | **参考输入形态** | 见 §4.6 |
| C14 | 🟡 | 参考/视频编码器是否共享权重 | — |
| C15 | 🟡 | register token 数量 K | — |
| **C18** | 🔴 | **推理形态（流式 / 非流式）** | ⚠️ **未定**（2026-09-17 用户指出这是决策不是既定）；概念澄清见 6 号 §2.6 |
| C19 | 🟡 | 动作长度（定长 / 不定长）| — |
| C20 | 🟡 | 一个视频多个动作 | — |
...
>
> 四种任务范式对比（定长识别 / TAD / TAS / OAD）见 6 号 §2.6。
