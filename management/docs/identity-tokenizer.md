...
```mermaid
flowchart TD
    R["参考输入（用户事先拍摄）<br/>单图 / 多视角图 / 短视频"] --> E0[身份编码器]
    ...
    E --> J["解码器<br/>decode(w_identity, λ_t)"]
    F --> J
    L --> J
    J --> K[重建视频帧]
```

> ⚠️ **上图含一处待解**：身份已由参考输入提供，则**视频内的 TIV 是否还需要**、**TV 如何获得多帧运动上下文**——两个问题未定，决定 T-A3 的 SIF 是否改写。详见 `openspec/changes/identity-action-tokenizer/design.md` T-A5c（架构 §4.6 C24）。
...
> **v1 窗口假设与待定项 12–14 的关系**：v1 先按**定长 16 帧窗口**建模（TivTok 口径，便于与 UCF101 阶段 A 对齐）。但真实监控里“一个窗口 = 一个标签”不成立——一个视频常含多个动作，时长跨度达 3 个数量级（打哈欠 ~1 s ↔ 睡觉 ~30 min）。
>
> **有利条件**：TV token 与 `λ_m(t)` 都是**逐帧**的 → 天然支持“逐帧标签 → 后处理成段”，不受“窗口 = 一个动作”限制；缺的是**时序后处理层**（中值滤波 / Viterbi / 最小段长约束）与**多时间尺度**（短窗+长窗分层）。
>
> 详见 [6 号《系统架构》§4.5](./architecture.md)（含四种任务范式对比：定长识别 / TAD / TAS / OAD）。

| 15 | **身份 embedding 模型** | DINOv2 / DINOv3 / CLIP·SigLIP / SuperAnimal / 猫专用 Re-ID | **默认候选 DINOv2，未实测**（在 `registry-retrieval` 实施时实测）|
| 16 | **向量库** | numpy 暴力检索 / FAISS | numpy 暴力检索（登记库仅几十个向量）|
| 17 | **参考输入形态** | 单图 / 3–5 张多视角图 / 参考短视频 | 三种均允许，推荐多视角 |
| 18 | **参考输入架构连带问题** | 视频内 TIV 去留 / TV 时序上下文 / 编码器是否共享权重 / 多参考聚合 | **待裁定**（阻塞 T-A3，详见该 change design T-A5c）|
