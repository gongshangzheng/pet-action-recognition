...
**图的读法（符号对照）**：见 [6 号 §2.4](./architecture.md) 的对照表（`w_identity` = 身份向量、`λ_j` = 动作系数（每 t 帧一份）、`V` = 正交运动基）。
...
**粒度够用**：0.13 s/点（@30fps）远细于段边界要求（±1.5 s）。详见 6 号 §2.7.3（C09 已定）。
...
>
> **动作识别模型的能力分层（A0–A5）** 见 6 号 §2.7.1——主流模型停在 A1（定长片段 → 1 个标签，无时间戳）；要给时间戳/多段需 A3/A4，而我们需要 A4 级输出但只有弱标注。
