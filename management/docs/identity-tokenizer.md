...
| ~~纯 register tokens~~ | ⚠️ **本条已修正（2026-09-17）**：否定的其实是"把 register 当**辅助**手段"（Darcet 用它吸收 ViT 全局 artifact，不承担读出职责）；**register tokens 作为读出载体已被采用**（见 §4.6）|
...
```mermaid
flowchart TD
    R["参考输入<br/>单图 / 多视角图 / 短视频"] --> RP[patchify]
    RP --> RT["⊕ 可学习 register tokens"]
    RT --> E0["编码器（与视频编码器同构，倾向共享权重）"]
    E0 --> L["只保留 register tokens<br/>K 个 · 身份表示（固定）"]
...
**读出机制：register tokens（单图 / 多图 / 视频共用一套结构）**（2026-09-17）

```
参考输入（单图/多图/视频）→ patchify
   ↓ 拼接可学习 register tokens
[register tokens ⊕ 输入 tokens] → 编码器 → 只保留 register tokens
   ↓  身份表示（固定 K 个 token，与输入形态/长度无关）
```

| 优点 | 说明 |
|---|---|
| **结构统一** | 单图/多图/视频走同一条路，形态差异只在"输入 token 多少" |
| **天然定长** | 任意长度 → 固定 K 个 token（学出来的聚合，强于池化）|
| **细节更全** | K 个 token 可分工（花纹/颜色/体型）；全局池化会把空间信息平均掉——直指"完整身份信息" |
| **多参考天然聚合** | register 跳图像/帧做注意力，**不需额外池化策略** |
| 先例 | TiTok 的 latent tokens、TivTok（扩到视频）、FlexTok 的 register tokens |

> 与 FLOAT 的差异（**改进点**）：FLOAT/LIA 用**全局平均池化**得 `w_identity`（信息压扁）；register 读出保留更多细节。
>
> ⚠️ **待定**：① `w_identity` 形状 vs 加法分解（倾向**解码器 cross-attention 读出**，形状自由）② 参考/视频编码器是否共享权重（倾向共享 + 两组 register）③ register token 数量 K。详见该 change design T-A5。
...
| 17 | **参考输入形态** | 单图 / 3–5 张多视角图 / 参考短视频 | **三种均允许，共用一套读出结构**——形态差异只在输入 token 数（2026-09-17）|
| 18 | 身份读出机制 | register tokens 读出 / 全局池化（FLOAT 做法）| **已定：register tokens**（2026-09-17）——详见 §4.6；优于池化（细节更全、多参考天然聚合）|
| 19 | **`w_identity` 形状 vs 加法分解** | ⓐ register 投影成单个 d 维 / ⓑ 运动也 K 个 token / ⓒ 解码器 cross-attention 读出 | **倾向 ⓒ**（形状自由，两边各自发展）|
| 20 | 参考编码器 vs 视频编码器共享权重 | 共享 / 分开 | **倾向共享**（结构已统一）+ 两组 register tokens（身份组/运动组）|
| 21 | **register token 数量 K** | — | 新超参（**非**原 N_TIV，用途不同）；待消融 |
| 22 | **`λ` 的时间粒度** | 接受 4 帧粒度（随 t=4 tubelet）/ tubelet 序列 → 逐帧头 / 动作分支用 t=1 patch | **待定**（3D patchify 带来的张力，见 §4.2）|
| 23 | **patch 尺寸 t × p × p** | t ∈ {2,4,8}；p ∈ {8,16} | t=4, p=8（AdapTok 口径）为起点，做消融 |
| 24 | **解码器是否加参考特征路径**（FLOAT 的 `feats`）| ⓐ 补（忠实 FLOAT）/ ⓑ 不补（靠容量闸门+审计）/ ⓒ 只中低分辨率 | **待裁定**（⚠️ 影响分离强度：不加则外观只能从压缩身份码出，运动通道更有动机携带外观）|
| 25 | **α 的时序上下文** | ⓐ FLOAT 忠实版（逐帧单帧算，无语境）/ ⓑ 加时序上下文（3D tubelet / 时序注意力）| 3D tubelet（t=4）是自然折中：自带 t 帧上下文且保住 per-position α |
