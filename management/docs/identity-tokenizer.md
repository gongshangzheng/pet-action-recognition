...
**InfoNCE 已移出主监督**（用户裁定 2026-09-17）：
- ❌ 它是**判别式**目标，只保证“分得开”、不保证“装得下”——2 维嵌入即可完美区分 2 只猫，但那 2 维几乎不含身份信息
- ✅ 我们要的是**完整**身份信息，**只有重建能保证**
- **事实依据**：FLOAT 全文 `infonce/contrastive` **0 次命中**；其损失只有 L1 + VGG 多尺度感知 + 面部部件感知 + 对抗 + FSM；身份评测用的 CSIM 是**指标**不是损失
- ⬇️ 降为**可选度量学习增强**（仅当身份嵌入要用于检索且度量不足时启用）；Re-ID 需求由登记-检索路线（§2 路线一）承担
...
| 10 | 身份监督形式 | — | **已定：参考输入 + 重建（不用 InfoNCE）**（2026-09-17）——见 §4.6 |
