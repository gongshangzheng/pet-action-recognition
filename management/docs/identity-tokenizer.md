...
| ~~纯 register tokens~~ | ⚠️ **本条已修正（2026-09-17）**：否定的其实是"把 register 当**辅助**手段"（Darcet 用它吸收 ViT 全局 artifact，不承担读出职责）。此后一度简化为"逐图编码后平均"，**又经用户裁定回退**（平均无法跨视角融合）；最终采用 **register tokens 联合融合 → 投影成单向量**（见 §4.6）|
...
| 17 | **参考输入形态（初版只支持一种）** | **A. 高质量多图**（3–5 张）/ **B. 参考视频**（一段）| **待决策**（2026-09-17）——**A** 原理最干净（静态图与「身份=不变量」对齐，**无参考运动污染**）、质量可控；**B** 用户成本最低、与视频编码器同构，但有**参考运动污染**。两者**共用同一套 register 结构** → 不阻塞架构 |
| 18 | 身份读出机制 | register tokens 联合融合 / 逐图编码后平均 | **已定：register tokens**（2026-09-17）——平均是背包式聚合、无法跨视角融合；register 融合后投影成单向量以保持加法分解简单。详见 §4.6 |
| 19 | `w_identity` 形状 vs 加法分解 | ⓐ register 融合后投影成单个 d 维 / ⓑ 保留 K 个 token + 解码器 cross-attention | **选 ⓐ**（融合能力来自 register，解码接口保持 FLOAT 式简单）；ⓑ 作瓶颈受限时的升级路径 |
| 20 | 参考编码器 vs 视频编码器共享权重 | 共享 / 分开 | **倾向共享**（须两次独立前向）+ **两组 register tokens**（身份组 / 运动组）|
| 21 | **register token 数量 K** | — | 待消融（K 大→信息全但冗余，K 小→瓶颈）|
