...
author: 郑鑫裕
date: 2026-09-17
tags: [模型设计, TivTok, FLOAT, LIA, AdapTok, SoftVQ, 正交运动基, UCF101, 身份-动作解耦]
summary: 视频动作识别模型的完整设计——AdapTok 基座 + TivTok SIF 双 token 骨架 + SoftVQ 量化 + FLOAT/LIA 正交运动基；两阶段训练（UCF101→猫语料）、损失设计、评测矩阵与待定选择
id: 8
---
...
| **SoftVQ-VAE** | arXiv 2412.10958 | **量化器**：连续软量化，无码本塌缩，线性探针表现优 |
...
| 模型内 VQ 码本 | 由离线聚类伪标签替代；若仍需量化则用 SoftVQ（软分配，无塌缩） |
...
flowchart TD
    ...
    E --> G[SoftVQ 量化<br/>连续软分配]
    F --> G

    F --> H["正交运动基分解<br/>z_t = Σ λ_m·v_m"]
    H --> I["λ_m 基元强度序列<br/>可解释动作表示"]

    G --> J["解码器<br/>输入 TIV + TV_t"]
    J --> K[重建视频帧]

    E --> L["身份嵌入<br/>池化 + track InfoNCE"]
```

**数据流说明**：TIV 与 TV 共享同一个编码器，通过 attention scope 分离；量化后两者拼接入解码器——第 t 帧只由 `TIV + TV_t` 重建（TivTok 的 Invariant Broadcasting）。
...
| 项 | 取值 |
|---|---|
| 形式 | 软分配：`q(z|x) = softmax(-‖ẑ-C‖²/τ)`，`z = q(z|x)·C` |
| 温度 | τ = 0.07 |
| 码本 | 8192 × 32（可选乘积量化分组） |
| 额外损失 | **无 codebook loss、无 commitment loss** |

**为什么不用硬 VQ**：硬 VQ 的码本利用率实测可低至 0.2%（塌缩）；SoftVQ 是连续软分配，天然全码本参与。更关键的是——**SoftVQ 的线性探针精度在所有 token 数下都优于 VQ 与普通自编码器**，而线性探针正是我们的验收闸门。

**副产品**：软分配等价于 soft K-Means，天然适配下游聚类（可直接对潜向量做 GMM 聚类）。
...
| # | 选择项 | 选项 | 当前倾向 |
|---|---|---|---|
| 1 | 编码器初始化 | AdapTok 权重 / SoftVQ 权重 / 从零 | AdapTok 权重（同形 + MIT） |
| 2 | 量化器 | SoftVQ / FSQ / VQ | SoftVQ（探针证据 + 无塌缩） |
...
| SoftVQ-VAE 仓库 | **无 LICENSE 文件** | ⚠️ 只移植算法、自写实现 |
