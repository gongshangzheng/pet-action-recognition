...
> **已决 / 不适用**：自适应 token 预算（v1 暂不适用）· `λ` 的时间粒度与时序上下文（并入「patch 尺寸」推论）· 对齐教师 DeRA（v1 不做）· **参考输入形态（已决：参考视频）** · **推理形态（已裁定：分段流式 + 判别非流式，见 6 号实时形态一节）**。（「身份 embedding 模型」「向量库」为检索相关，见 6 号 §2）
