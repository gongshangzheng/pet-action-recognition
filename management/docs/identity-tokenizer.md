...
> ⚠️ **AdapTok 只能借代码、不能借结构**：可复用 **3D patchify 实现 / transformer block / 训练循环 / 数据管线 / 12L·768d 规模 / 训练配方**；**不能复用** **block-causal 掩码**（为自回归设计，我们离线能看整段 → 白白限制）、**SVQ 量化器**（已弃）、**1D 全局 latent 读出**（我们要逐 t 帧块）、**block-mask 预算**（已弃）、**解码器量化输入接口**（我们喂连续 latent）。详见 6 号 §2 已决项（λ 粒度与时序上下文，已并入「patch 尺寸」推论）。
...
**由此得到那个根本矛盾（原「动作长度」待定项的由来；已裁定：**段不定长**，此矛盾仅存在于**模型输入窗口的尺度选择**）**：
```
窗口覆盖长 → 时间分辨率低（快动作丢失）
窗口覆盖短 → 长动作看不全
→ 单一时间尺度无法两头兼顾
...
3. 若要「**模型内部**处理任意长序列」→ 需因果/流式或长窗注意力 → 分段组件需流式（见 §11）；窗口跨度属输入尺度选择（段形态已裁定不定长）
...
> **修订（用户提议）**：参考与输入视频**共用同一套结构与参数**（FLOAT 即如此），差别只在**取哪部分输出**——参考侧取 register（身份），输入侧取 t 帧块系数（动作）。完整版见下文 §4.6b。
>
...
### §5.1 训练与使用：完整流程细节（自 6 号并入，与上文阶段 A/B 图互补）
...
> 6 号 §2 不再列模型内部条目（用户裁定）；跨环节条目（夜间红外 / 一个视频多个动作等）见 6 号 §2。**内容以 §4 为准**。
