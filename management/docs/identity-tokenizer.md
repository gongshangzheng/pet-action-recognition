...
**图的读法（符号对照）**：见 [6 号 §5.4](./architecture.md) 的对照表（`w_identity` = 身份向量、`λ_j` = 动作系数（每 t 帧一份）、`V` = 正交运动基）。
...
|---|---|
| 夜间红外处理方案 | 输入域，跨环节 |
| 多猫场景掩码 | 影响身份训练的「单猫片段」切分 |
| 视频编码器代码基座 | 见 §4.2 |
| 编码器初始化 | 决定分阶段冻结是否可行（见 §5）|
| patch 尺寸 `t×p×p` | 见 §4.2 |
| 输入规格（分辨率 / 帧数）| — |
| 正交基 M / d / 符号 | 见 §4.5 |
| 训练粒度（A1 冻结 / A1+A2）| 见 §5 训练流程（依赖「编码器初始化」）|
| 参考/视频编码器是否共享权重 | — |
| register token 数量 K | — |
| 动作长度（定长 / 不定长）| — |
| 一个视频多个动作 | — |

> 本表只列**模型层相关**条目，与 6 号 §2「🟡 待定 · 模型级」一致（2026-09-17 起不再使用 C 编号）；**内容以 §4 为准**。
...
**分档原则见 [6 号 §8.1](./architecture.md)**：限制「使用范围」（🟡 非商用）与限制「衍生作品传染」（🔴 GPL）是两回事，不可同档处理。
