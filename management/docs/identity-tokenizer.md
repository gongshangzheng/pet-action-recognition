...
> ⚠️ **AdapTok 只能借代码、不能借结构**（2026-09-17）：可复用 **3D patchify 实现 / transformer block / 训练循环 / 数据管线 / 12L·768d 规模 / 训练配方**；**不能复用** **block-causal 掩码**（为自回归设计，我们离线能看整段 → 白白限制）、**SVQ 量化器**（已弃）、**1D 全局 latent 读出**（我们要逐 t 帧块）、**block-mask 预算**（已弃）、**解码器量化输入接口**（我们喂连续 latent）。详见 6 号 §2.3 C09/C10。
