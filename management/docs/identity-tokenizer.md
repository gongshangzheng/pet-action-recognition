...
> ⚠️ **AdapTok 只能借代码、不能借结构**（2026-09-17）：可复用 **3D patchify 实现 / transformer block / 训练循环 / 数据管线 / 12L·768d 规模 / 训练配方**；**不能复用** **block-causal 掩码**（为自回归设计，我们离线能看整段 → 白白限制）、**SVQ 量化器**（已弃）、**1D 全局 latent 读出**（我们要逐 t 帧块）、**block-mask 预算**（已弃）、**解码器量化输入接口**（我们喂连续 latent）。详见 6 号 §2.3 C03/C04。
...
### §4.2a 聚合粒度：为什么每 t 帧一份动作系数（已决，并入 C05 推论）
...
>
> ⚠️ 本节**不再维护本地编号**（历史上用过本地 `1–25`，与架构的 `C` 编号并列造成「同一决策两个 ID」，已废弃）。下表只列**模型层相关**的 C-ID 与补注（编号已与 6 号 §2 同步，2026-09-17 压缩重排）；**内容以 §4 为准**。
...
| C06 | 🟡 | 输入规格（分辨率 / 帧数）| — |
| C07 | 🟡 | 正交基 M / d / 符号 | 见 §4.5 |
| C08 | 🟡 | 训练粒度（A1 冻结 / A1+A2）| 见 §5 训练流程 |
| C09 | 🟡 | 参考/视频编码器是否共享权重 | — |
| C10 | 🟡 | register token 数量 K | — |
| C13 | 🟡 | 动作长度（定长 / 不定长）| — |
| C14 | 🟡 | 一个视频多个动作 | — |

> **已决 / 不适用（不再占号）**：自适应 token 预算（v1 暂不适用）· `λ` 的时间粒度与时序上下文（并入 C05 推论）· 对齐教师 DeRA（v1 不做）· **参考输入形态（已决：参考视频）** · **推理形态（已裁定：分段流式 + 判别非流式，见 6 号实时形态一节）**。（C11/C12 为身份检索相关，见 6 号 §2）

> **v1 窗口假设与 C13/C14 的关系**：v1 先按**定长 16 帧窗口**建模（便于与 UCF101 阶段 A 对齐）。但真实监控里「一个窗口 = 一个标签」不成立——一个视频常含多个动作，时长跨度达 3 个数量级（打哈欠 ~1 s ↔ 睡觉 ~30 min）。
>
