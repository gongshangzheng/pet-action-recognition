...
| ~~纯 register tokens~~ | ⚠️ **本条已修正（2026-09-17）**：否定的其实是"把 register 当**辅助**手段"（Darcet 用它吸收 ViT 全局 artifact，不承担读出职责）。此后一度采用 register 读出，又经 FLOAT 源码二次核对后**简化为 FLOAT 式全局向量**（见 §4.6）|
...
```mermaid
flowchart TD
    R["参考输入<br/>单图 / 多视角图 / 短视频"] --> E0["身份编码器（FLOAT 式）<br/>只吃参考 · 与运动分支独立前向"]
    E0 --> L["512 维全局向量<br/>多图/视频 → 逐图编码后平均"]
...
FLOAT 的身份提取就是**卷积编码器 + 全局池化成一个向量**，没有 register / query token：

| 参考形态 | 处理 |
|---|---|
| 单张图 | 直接编码 → 512 维向量（= FLOAT 原样）|
| 3–5 张多视角图 | 逐图走**同一个**编码器 → 向量**平均** |
| 一段参考视频 | 逐帧（或抽帧）编码 → 向量**平均** |
...
→ **register tokens 的"分工保留更多细节"优势在本用途下用不上**，故已取消（决策记录见该 change design T-A5）。

**⚠️ 保留的正确性要求**：身份分支与运动分支**必须独立前向**（身份编码器只吃参考、运动编码器只吃视频）——否则"身份不接触视频"的硬保证失效。
...
| 18 | 身份读出机制 | register tokens / **FLOAT 式全局向量** | **已定：FLOAT 式**（2026-09-17）——身份码只要"条件"不要"档案"；多图/视频取平均，详见 §4.6 |
| 19 | ~~`w_identity` 形状 vs 加法分解~~ | — | **已消解**：`w_identity` 就是单个 512 维向量，加法分解直接可用 |
| 20 | 参考编码器 vs 视频编码器共享权重 | 共享 / 分开 | **倾向共享**（两次独立前向，各自的输入不同）|
| 21 | ~~register token 数量 K~~ | — | **已取消**（随 register 方案一并移除）|
...
| 24 | **解码器是否加参考特征路径**（FLOAT 的 `feats`）| ⓐ 补 / ⓑ 不补 / ⓒ 折中 | **已定 ⓑ 不补**（2026-09-17）——目标不是重建质量；不补反使身份码**更完整**（外观必须从身份码出），代价是泄漏压力略高 → 双向审计把门，不过再给 λ 加对抗约束 |
