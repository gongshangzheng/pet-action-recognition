...
> ⚠️ **数据构造硬约束**：**参考与驱动必须来自不同视频 / 不同片段**——同一段会双向出问题：① 参考里已含该动作 → **动作信息泄漏进身份潜变量**；② 两者动作一致 → **掩盖泄漏**（审计看不出）。猫语料配对优先跨天；UCF101 阶段用**同一 group（同人）的不同 clip**。
...
```
跨片段 S→D 重建（参考 S 与驱动 D 来自**同一身份的不同片段**；禁止同段）
...
| 训练：跨片段 S→D 重建 | 同左（**UCF101 用同一 group〔同人〕的不同 clip 自动构造参考，无需人工标注**）|
...
**数据构造**（零标注，**同身份的不同片段**各取一份——禁止同段）：
```
同一只猫的两段：一段当【参考】（提供身份）、一段当【驱动】（提供动作）
⚠️ MUST 来自【不同视频 / 不同片段】——否则 ① 参考里已含动作，λ 可以偷懒；② **动作信息会泄漏进身份潜变量**，且两者一致会**掩盖泄漏**（审计失效）
```
