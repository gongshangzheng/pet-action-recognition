    ...
    <n-card size="small">
      ...
      <div class="live-layout">
        ...
        <div class="live-right">
          <!-- 真直播模式（帧级同步） -->
          <CanvasPlayer v-if="isDemoMode && liveStreamUrl" ref="canvasPlayerRef" :src="liveStreamUrl" />
          <!-- 静态视频播放 -->
...
import { NCard, NSpin, NList, NListItem, NThing, NTag, NEmpty, NText, NButton, NSpace, NSelect, NInputNumber, NScrollbar, useMessage } from 'naive-ui'
...
function startInfer() {
  ...
  // demo 模式走 CanvasPlayer（帧级同步），无需单独 SSE
...
function stopInfer() {
  ...
  if (canvasPlayerRef.value) canvasPlayerRef.value.disconnect()
  liveStreamUrl.value = ''
...
function playDemo() {
  ...
  liveStreamUrl.value = `/api/live/demo/live_stream?video_name=${encodeURIComponent(selectedFile.value)}&model_id=${encodeURIComponent(modelId)}&stride_sec=${strideSec.value}&clip_sec=1&device=${device.value}`
