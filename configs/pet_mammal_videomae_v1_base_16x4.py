# 哺乳动物动作识别 VideoMAE v1 配置（ViT-B, 16 frames, 224px）
# 适配 pet_action_mammal_v0（7 训练类）。
#
# 与 VideoMAEv2 条目的定位差异：本条目 backbone 来自 OpenMMLab 的
# VideoMAE v1 K400 掩码预训练→微调权重（纯 K400 口径），用于对照
# 「K400 掩码预训练（v1） vs K710 蒸馏（v2）」在小数据四足数据上的差异。
#
# finetune：backbone init_cfg 加载 K400 ckpt（prefix=backbone. 只取权重主干），
# cls_head 7 类随机初始化。勿用 train_model.py --pretrained（整模加载 head 冲突）。
